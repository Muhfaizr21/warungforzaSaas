...
const ReadyStockPage = () => {
    ...
    const [localSearch, setLocalSearch] = useState(''); // Immediate UI state
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            const params = {
                ...
                search: searchTerm,
                limit: 200 // Ensure we get all products for client-side pagination
            };
            try {
    ...
    return (
        <div className="bg-[#030303] pt-16 pb-20 min-h-screen">
            <SEO
                title="Ready Stock Collectibles"
                description="Browse our immediate collection of premium action figures and statues. In-stock items ready for global shipping."
            />
            {/* Minimalist Header */}
            <div className="relative h-48 flex items-center overflow-hidden mb-8 border-b border-white/5">
                <div className="absolute inset-0 bg-[url('/images/horror-hero.jpg')] bg-cover bg-center opacity-10 grayscale brightness-50"></div>
                ...
                <div className="container relative z-10 w-full">
                    <h1 className="text-white uppercase italic">
                        {t('readystock.title').split(' ')[0]} <span className="text-rose-600">{t('readystock.title').split(' ')[1] || 'STOCK'}</span>
                    </h1>
                    <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.4em] mt-2">{t('readystock.subtitle')}</p>
                </div>
            </div>
            ...
            <div className="container">
                ...
                <div className="mb-8 flex flex-col items-center gap-4 z-20 relative">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-4 w-full">
                        <div className="relative group w-full">
                            ...
                            <input
                                type="text"
                                placeholder={t('readystock.searchPlaceholder')}
                                className="input-standard pl-12"
                                onChange={(e) => setLocalSearch(sanitizeInput(e.target.value))}
                                value={localSearch}
                            />
                        </div>
                        <p className="hidden md:block text-white/30 text-[10px] font-black uppercase tracking-widest whitespace-nowrap">
                            <span className="text-white">{totalItems}</span> {t('readystock.productsCount')}
                        </p>
                    </div>
                ...
                <main>
                    {loading ? (
                        <div className="py-8">
                            <SkeletonGrid count={productsPerPage} />
                        </div>
                    ) : (
                        <>
                            {currentProducts.length === 0 ? (
                                <div className="py-20 text-center border border-white/5 bg-[#050505] rounded-lg">
                                    <p className="text-white/40 text-xs font-black uppercase tracking-widest">{t('readystock.noProducts')}</p>
                                    <button
                            ...
                            {totalPages > 1 && (
                                <div className="mt-20 flex justify-center items-center gap-2">
                                    <button
                                        ...
                                        className="px-4 py-2 bg-[#111] text-white/50 text-[10px] font-black uppercase tracking-widest rounded-sm hover:bg-white hover:text-black disabled:opacity-20 transition-all border border-white/5"
                                    >
                                        {t('common.previous')}
                                    </button>
                                    ...
                                    {[...Array(totalPages)].map((_, i) => (
                                        <button
                                            ...
                                            className={`w-10 h-10 rounded-sm text-[10px] font-bold transition-all ${currentPage === i + 1
                                                ? 'bg-rose-600 text-white shadow-xl shadow-rose-600/20'
                                                : 'bg-[#111] text-white/40 hover:bg-white/10'
                                                }`}
                                        >
                                            {i + 1}
                                        </button>
